...
function garage(array) {
    ...
    for (let line of array) {
        ...
        if (allGarages.hasOwnProperty(garageNumber)) { //проверяваме дали номера на гаража съществува
            allGarages[garageNumber].push(carInfo); // // ако съществува, добавяме информацията към него
        } else {
            allGarages[garageNumber] = [carInfo]; // ако не съществува, добавяме номера на гаража и инофмрацията към него
        }      
    }

    let entries = Object.entries(allGarages); // вадим двойките елемнти - ключ и стойност
